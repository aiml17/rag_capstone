"""
Preliminary Evaluation Script v11
...
- Skips examples with context+question > 4000 chars (uses backup samples)
...
MAX_CONTEXT_LENGTH = 20000  # Skip examples with context+question > this (Groq limit ~25k)
...
class PreliminaryEvaluator:
    ...
    def _load_checkpoint(self, config: Tuple) -> Dict:
        ...
        if os.path.exists(legacy_path):
            ...
            print(f"  📁 Migrated checkpoint to numbered format: {numbered_name}")
            return data

        return {"completed": [], "results": [], "skipped_samples": []}
...
Answer:"""
    ...
    def load_data(self):
        print(f"\n📂 Loading {self.domain} data...")
        data = self.loader.load_domain(self.domain, "test")
        indices = list(range(len(data)))
        random.shuffle(indices)
        # Load 3x samples to have backups for skipped examples
        sample_count = NUM_EXAMPLES * SAMPLE_POOL_MULTIPLIER
        self.samples = [data[i] for i in indices[:sample_count]]
        print(f"  Loaded {len(self.samples)} examples as pool (from {len(data)} total)")
    ...
    def evaluate_example(self, sample_idx: int, sample: Dict, config: Tuple, result_idx: int) -> ExampleResult:
        """Evaluate a single example. result_idx is the index in results (0 to NUM_EXAMPLES-1)."""
        config_type, chunking, method, llm = config
        ...
        print(f"Context length: {total_length}")
        if total_length > MAX_CONTEXT_LENGTH:
            return ExampleResult(
                ...
                is_valid=False, is_rate_limited=False, is_skipped=True,
        ...
        if is_rate_limited:
            return ExampleResult(
                ...
                gt_completeness=gt_scores['completeness'], gt_adherence=gt_scores['adherence'],
                binary_pred=0, binary_gt=1 if gt_scores['adherence'] >= 0.5 else 0,
                is_valid=False, is_rate_limited=True, is_skipped=False, error="Rate limit hit"
            )
        ...
        if not response:
            return ExampleResult(
                ...
                gt_completeness=gt_scores['completeness'], gt_adherence=gt_scores['adherence'],
                binary_pred=0, binary_gt=1 if gt_scores['adherence'] >= 0.5 else 0,
                is_valid=False, is_rate_limited=False, is_skipped=False, error="Empty response"
            )
        ...
        try:
            ...
            return ExampleResult(
                ...
                is_valid=True, is_rate_limited=False, is_skipped=False, error=""
            )
        except Exception as e:
            return ExampleResult(
                example_idx=result_idx, dataset_id=dataset_id, question=question,
                ...
                gt_completeness=gt_scores['completeness'], gt_adherence=gt_scores['adherence'],
                binary_pred=0, binary_gt=1 if gt_scores['adherence'] >= 0.5 else 0,
                is_valid=False, is_rate_limited=False, is_skipped=False, error=str(e)
            )
    ...
    def evaluate_config(self, config: Tuple) -> Optional[ConfigResult]:
        ...
        print(f"\n{'='*60}")
        print(f"📋 [{config_idx:03d}] {config_type}/{chunking}/{method}/{llm.split('/')[-1]}")
        ...
        while result_idx < NUM_EXAMPLES and sample_idx < len(self.samples):
            ...
            if result.is_skipped:
                print(f"⚠ Too large - skipping. Context length {result.error}")
                skipped_samples.add(sample_idx)
                self._save_checkpoint(config, {
                    "completed": list(range(len(results))),
                    "results": results,
                    "skipped_samples": list(skipped_samples)
                })
                sample_idx += 1
                continue  # Try next sample, don't increment result_idx
            ...
            # Valid result (or failed but not skipped/rate-limited)
        ...
        print(f"\n{'=' * 60}")
        print(f"\n  📊 Results: TRACe={avg_trace:.3f} (R={avg_rel:.2f}, U={avg_util:.2f}, C={avg_comp:.2f}, A={avg_adh:.2f})")
        print(f"  ✅ Valid: {len(valid)}/{len(results)}, Skipped: {len(skipped_samples)}")
    ...
    def run_evaluation(self):
        print("\n" + "=" * 70)
        print(f"🚀 PRELIMINARY EVALUATION - {self.domain.upper()}")
        print("=" * 70)
        ...
        try:
            ...
            print(f"💾 Saving all results before exit...")
            self._save_all_results()
            print(f"\n📋 To resume, run the same command again.")
            sys.exit(0)
    ...
    def _save_all_results(self):
        ...
        for config in self.all_configs:
            ...
            if len(checkpoint.get("results", [])) >= NUM_EXAMPLES:
...
def main():
    parser = argparse.ArgumentParser(description="Preliminary Evaluation v11")
